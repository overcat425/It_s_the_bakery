...
public class Player : MonoBehaviour
{
    ...
    public int[] playerDesserts = { 0, 0 };  // 0ÀÌ Donut, 1ÀÌ Cake
    ...
    void Move()
    {
        camY = Camera.main.transform.eulerAngles.y;   // Ä«¸Þ¶ó -45 È¸Àü¿¡ ´ëÇÑ ÇØ°áÃ¥
        Quaternion camRot = Quaternion.Euler(0f, camY, 0f);   // Ä«¸Þ¶óÀÇ YÃà¸¸ ¹ÞÀº µÚ,
        speed = (2 + GameManager.instance.upgradeScript.moveSpeed * 0.3f);
        inputVec = GameManager.instance.joystickScript.inputVec;
        moveVec = camRot *  new Vector3(inputVec.x, 0, inputVec.y);   // ÇÃ·¹ÀÌ¾îÀÇ ÀÌµ¿°ª¿¡ °öÇØÁÜ
    ...
    //        if (GameManager.instance.customerMoving.customerObjects.Count <= 0) return; //¼Õ´ÔÀÌ ¾øÀ»‹š´Â µ¿ÀÛÇÏÁö ¾ÊÀ½
    ...
    {
        if (other.gameObject.CompareTag("Money"))   // µ· È¹µæ
        {
    ...
    IEnumerator PlayerGetMoney(int stackCount, MoneyManager moneyManager)
    {
        ...
        {
            Transform money = moneyManager.moneyStack.Pop();    // money ½ºÅÃ¿¡¼­ Pop
            GameManager.instance.GetMoney(10);          // µ· È¹µæ
            money.gameObject.SetActive(false);
            yield return new WaitForSeconds(0.01f);     // µ· È¹µæ °£°Ý
        }
        Tuto(3);
    }
    ...
    //{
    //    GameObject[][] dessertObjects = { donuts, cake }; // 0: µµ³Ó, 1: ÄÉÀÌÅ©
