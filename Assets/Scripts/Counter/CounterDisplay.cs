...
public class CounterDisplay : MonoBehaviour // 상품 계산 스크립트
{
    ...
    private void OnTriggerStay(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (timer > 0.08f && (playerHand.hands[0].Count > 0 || playerHand.hands[1].Count > 0))
            {
                MoveDessert();
                timer = 0f;
            }
    ...
    void MoveDessert()
    {
        ...
        for (int i = 0; i < playerHand.hands.Length; i++)
        {
            if (playerHand.hands[i].Count > 0)
            {
                float above = i == 0 ? 0.08f : 0.12f;
                Transform dessert = playerHand.hands[i].Pop();
